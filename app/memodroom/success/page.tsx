  ...
  if (error) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/" className="mt-6 inline-block text-sm text-[#d9a016] hover:underline">
          Back to home
        </Link>
      </div>
    );
      ...
      {currentStatus === "failed" ? (
        <div className="text-center">
          ...
          <a
            ...
            className="text-sm text-[#d9a016] hover:underline"
          >
            hello@memodroom.com
          </a>
        </div>
        ...
        <>
          ...
          <div className="text-center mb-12">
            <p className="text-xs tracking-[0.3em] uppercase text-[#d9a016] mb-3">Memodroom</p>
            {currentStatus === "delivered" ? (
              <h1 className="font-serif text-4xl mb-3">Your Memo is ready.</h1>
            ) : (
          ...
          {currentStatus !== "delivered" && (
            <div className="mb-10">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-zinc-600">{progressPct}%</span>
              </div>
              <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#d9a016] rounded-full transition-all duration-1000 ease-out"
          ...
          <div className="space-y-0 mb-10">
            {STATUS_STEPS.filter((s) => s.status !== "failed").map((s) => {
              ...
              return (
                <div key={s.status} className="flex items-start gap-4 py-4 border-b border-white/5 last:border-0">
                  <div className="shrink-0 mt-0.5">
                    {isDone && currentStatus === "delivered" ? (
                      ...
                      <div className="w-6 h-6 rounded-full bg-[#d9a016] flex items-center justify-center">
                        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M2 6l3 3 5-5" />
                        </svg>
                      </div>
                    ) : isCurrent ? (
                      <div className="w-6 h-6 rounded-full border-2 border-[#d9a016]/40 flex items-center justify-center">
                        <div className="w-3 h-3 rounded-full border-2 border-[#d9a016]/30 border-t-[#d9a016] animate-spin" />
                      </div>
                    ) : (
                      <div className="w-6 h-6 rounded-full border-2 border-white/10 bg-white/2" />
                    )}
                  </div>
                  <div>
                    <p
                      className={`text-sm font-medium transition-colors ${
                        isDone || isCurrent ? "text-[#f5f0e8]" : "text-zinc-600"
                      }`}
                    >
                      {s.label}
                    </p>
          ...
          {currentStatus !== "delivered" && senderEmail && (
            <div className="p-4 bg-zinc-900/40 border border-white/5 rounded-xl mb-6">
              <p className="text-xs text-zinc-500 text-center leading-relaxed">
                We&apos;ll send it to{" "}
                <span className="text-zinc-300 font-medium">{senderEmail}</span>{" "}
          ...
          {isLong && currentStatus !== "delivered" && (
            <div className="p-4 bg-yellow-950/30 border border-yellow-600/20 rounded-xl mb-6">
              <p className="text-xs text-zinc-400 leading-relaxed text-center">
                Taking a bit longer than usual — check your email, it may already be there.{" "}
                <a href="mailto:hello@memodroom.com" className="text-[#d9a016] hover:underline">
                  Contact support
                </a>
              </p>
            </div>
          ...
          {currentStatus === "delivered" && order && (order.videoUrls?.length ?? 0) > 0 && (
            <div className="space-y-6">
              <h2 className="font-serif text-2xl text-center mb-2">Your Memo</h2>
              {order.videoUrls.map((url, i) => (
                <div
                  key={i}
                  className="border border-white/8 rounded-2xl overflow-hidden bg-zinc-900/20"
                >
                  {order.plan === "bundle" && (
                    <div className="px-5 py-3 border-b border-white/8">
                      <p className="text-xs text-zinc-500 font-medium">{BUNDLE_LABELS[i] || `Version ${i + 1}`}</p>
                    </div>
                  )}
                  <video
                    src={url}
